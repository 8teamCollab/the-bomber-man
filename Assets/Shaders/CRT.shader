...
Shader "Custom/CRT"
{
    Properties
    {
        ...
        _AberrationStrength("AberrationStrength", Float) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                if (_UseConvergence) {
                    float aberrationA = _AberrationStrength/1000;
                    
                    // Primary aberration
                    col.r = tex2D(_MainTex,float2(uv.x+aberrationA,uv.y+aberrationA)).x;
                    col.g = tex2D(_MainTex,float2(uv.x,uv.y-aberrationA*2)).y;
                    col.b = tex2D(_MainTex,float2(uv.x-aberrationA*2,uv.y)).z;

                    // Second, fainter aberration
                    float d_offset = _DoubleOffset/10;
                    float d_str = _DoubleStrength/10;
                    col.r += d_str*tex2D(_MainTex,d_offset*float2(0.025, -0.027)+float2(uv.x+aberrationA,uv.y+aberrationA)).x;
                    col.g += d_str*tex2D(_MainTex,d_offset*float2(0.022, -0.02)+float2(uv.x,uv.y-aberrationA*2)).y;
                    col.b += d_str*tex2D(_MainTex,d_offset*float2(0.02, -0.018)+float2(uv.x-aberrationA*2,uv.y)).z;
                }
